    ...
    <div class="container">
        <div class="card shadow">
            ...
            <div class="card-body overflow-auto">
                <form method="GET" action="{{ route('user-types.index') }}">
                    @csrf
        ...
        <div class="card shadow mt-3 mb-3">
            ...
            <div class ="card-body overflow-auto">
                    ...
                    <tbody>
                        @forelse ($data as $datum)
                            <tr>
                                ...
                                <td><a
                                        href="{{ route('user-types.show', ['user_type' => $datum->id]) }}">{{ sprintf('%08d', $datum->id) }}</a>
                                </td>
                                <td>{{ $datum->code }}</td>
                                <td>{{ $datum->name }}</td>
                                <td>{{ $datum->remarks }}</td>
                                ...
                                <td>
                                    <div class="form d-inline">
                                        <a href="{{ route('user-types.edit', ['user_type' => $datum->id]) }}"
                                            class="btn btn-sm btn-botejyu action-btn">
                                            <i class="fa-solid fa-pencil"></i>
                                        </a>
                                    </div>
                                    <form method="POST" class="form d-inline "
                                        action="{{ route('user-types.destroy', ['user_type' => $datum->id]) }} "
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                ...
                <div class="form d-inline">
                    <a href="{{ route('user-types.create') }}" class="btn btn-sm btn-botejyu mb-3 create-btn">
