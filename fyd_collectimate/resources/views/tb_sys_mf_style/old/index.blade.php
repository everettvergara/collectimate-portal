...
@section('content')
    <div class="container">
        <div class="card shadow">
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('styles.index') }}">
                    @csrf
        ...
        <div class="card shadow mt-3 mb-3">
            ...
            <div class ="card-body overflow-auto">
                <table class="table table-sm table-striped ">
                    <thead class="table-light-blue">
                        <tr>
                            ...
                            <td>@sortablelink('width', 'WIDTH')</td>
                            <td>@sortablelink('height', 'HEIGHT')</td>
                            <td>@sortablelink('path', 'Path')</td>
                            <td>Actions</td>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($data as $datum)
                            <tr>
                                ...
                                <td><a
                                        href="{{ route('styles.show', ['style' => $datum->id]) }}">{{ sprintf('%08d', $datum->id) }}</a>
                                </td>
                                <td>{{ $datum->code }}</td>
                                <td>{{ $datum->name }}</td>
                                <td>{{ $datum->width }}</td>
                                ...
                                <td>
                                    <div class="form d-inline">
                                        <a href="{{ route('styles.edit', ['style' => $datum->id]) }}"
                                            class="btn btn-sm btn-botejyu action-btn">
                                            <i class="fa-solid fa-pencil"></i>
                                        </a>
                                    </div>
                                    <form method="POST" class="form d-inline "
                                        action="{{ route('styles.destroy', ['style' => $datum->id]) }} " class="d-inline">
                                        @csrf
                                        @method('DELETE')
                ...
                <div class="form d-inline">
                    <a href="{{ route('styles.create') }}" class="btn btn-sm btn-botejyu mb-3 create-btn">
