...
@section('content')
    <div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 30px auto 0 auto;">
            <tbody>
                @if ($listing_image_floor_plans->isNotEmpty())
                    @if ($listing_image_floor_plans->count() === 1)
                        {{-- Single image full width --}}
                        <tr>
                            <td colspan="3"
                                style="padding: 15px; text-align: center; background-color: transparent !important;">
                                <img src="{{ public_path('storage/images/Original/' . $listing_image_floor_plans[0]->attachment) }}"
                                    alt=""
                                    style="
                                width: 100%;
                                height: 480px;
                    ...
                    @else
                        ...
                        @foreach ($listing_image_floor_plans->chunk(3) as $row)
                            <tr>
