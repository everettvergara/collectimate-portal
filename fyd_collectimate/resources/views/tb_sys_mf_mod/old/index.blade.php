    ...
    <div class="container">
        ...
        <div class="card shadow mt-3 mb-3">
            ...
            <div class ="card-body overflow-auto">
                <table class="table table-sm table-striped">
                    <thead class="table-light-blue">
                        <tr>
                            ...
                            <td>@sortablelink('mod_group', 'MOD Group')</td>
                        ...
                        @forelse ($data as $datum)
                            <tr>
                                ...
                                <td>
                                    @if ($datum->id != 0)
                                        <a
                                            href="{{ route('mods.show', ['mod' => $datum->id]) }}">{{ sprintf('%08d', $datum->id) }}</a>
                                    @endif
                                </td>
                                <td>{{ $datum->code }}</td>
                                <td>{{ $datum->name }}</td>
                                <td>{{ $datum->mod_group }}</td>
                                <td>
                                    @if ($datum->id != 0)
                                        <div class="form d-inline">
                                            <a href="{{ route('mods.edit', ['mod' => $datum->id]) }}"
                                                class="btn btn-sm mb-1 btn-botejyu action-btn">
                                                <i class="fa-solid fa-pencil"></i>
                                            </a>
                                        </div>
                                        <form method="POST" class="form d-inline "
                                            action="{{ route('mods.destroy', ['mod' => $datum->id]) }} " class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" value="Delete!"
                                                class="btn btn-sm mb-1 btn-danger action-btn"
                ...
                <div class="form d-inline">
                    <a href="{{ route('mods.create') }}" class="btn btn-sm btn-botejyu mb-1 create-btn">
