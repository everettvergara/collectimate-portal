...
use App\Http\Requests\tb_re_mf_developer_request;
use App\Models\tb_re_mf_developer;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class tb_re_mf_developer_controller extends Controller
{
    protected $route;
    protected $route_var;
    protected $title;
    ...
    {
        store_audit(Auth::user()->id, 'DEVELOPERS');
        $name = $r['name'];
        $data = tb_re_mf_developer::when(isset($name), function ($q) use ($name) {
            return $q->where('tb_re_mf_developer.name', 'like', '%' . $name . '%');
        })
            ->sortable()
    ...
    public function store(tb_re_mf_developer_request $r)
    {
        $validatedData = $r->validated();
        $datum = new tb_re_mf_developer();
    ...
    {
        $datum = tb_re_mf_developer::findOrFail($id);
    ...
    public function update(tb_re_mf_developer_request $r, $id)
    {
        $datum = tb_re_mf_developer::findOrFail($id);
        $validatedData = $r->validated();
        $datum->fill($validatedData);
