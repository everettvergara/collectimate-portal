...
use App\Http\Requests\tb_sys_mf_access_type_request;
use App\Models\tb_sys_mf_access_type;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class tb_sys_mf_access_type_controller extends Controller
{
    protected $route;
    protected $route_var;
    protected $title;
    ...
    {
        $this->route = 'access-types';
        $this->route_var = 'access_type';
        $this->title = 'Access Types';
        $this->view_path = 'tb_sys_mf_access_type';
    }

    public function index(Request $r)
    {
        store_audit(Auth::user()->id, 'ACCESS TYPES');
        $name = $r['name'];
        $data = tb_sys_mf_access_type::when(isset($name), function ($q) use ($name) {
            return $q->where('tb_sys_mf_access_type.name', 'like', '%' . $name . '%');
        })
            ->sortable()
    ...
    public function store(tb_sys_mf_access_type_request $r)
    {
        $validatedData = $r->validated();
        $datum = new tb_sys_mf_access_type();
    ...
    {
        $datum = tb_sys_mf_access_type::findOrFail($id);
    ...
    public function update(tb_sys_mf_access_type_request $r, $id)
    {
        $datum = tb_sys_mf_access_type::findOrFail($id);
        $validatedData = $r->validated();
        $datum->fill($validatedData);
