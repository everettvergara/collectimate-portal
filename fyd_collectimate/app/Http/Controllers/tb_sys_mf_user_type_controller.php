...
use App\Http\Requests\tb_sys_mf_user_type_request;
use App\Models\tb_sys_mf_user_type;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class tb_sys_mf_user_type_controller extends Controller
{
    protected $route;
    protected $route_var;
    protected $title;
    ...
    public function __construct()
    {
        ...
        $this->route_var = 'user_type';
        $this->title = 'USER TYPES';
        $this->view_path = 'tb_sys_mf_user_type';
    }

    public function index(Request $r)
    {
        store_audit(Auth::user()->id, 'USER-TYPES');
        $name = $r['name'];
        $data = tb_sys_mf_user_type::when(isset($name), function ($q) use ($name) {
            return $q->where('tb_sys_mf_user_type.name', 'like', '%' . $name . '%');
        })
            ->sortable()
    ...
    public function store(tb_sys_mf_user_type_request $r)
    {
        $validatedData = $r->validated();
        $datum = new tb_sys_mf_user_type();
    ...
    {
        $datum = tb_sys_mf_user_type::findOrFail($id);
    ...
    public function update(tb_sys_mf_user_type_request $r, $id)
    {
        $datum = tb_sys_mf_user_type::findOrFail($id);
        $validatedData = $r->validated();
        $datum->fill($validatedData);
