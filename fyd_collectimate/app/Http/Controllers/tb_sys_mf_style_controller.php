...
use App\Http\Requests\tb_sys_mf_style_request;
use App\Models\tb_sys_mf_style;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class tb_sys_mf_style_controller extends Controller
{
    protected $route;
    protected $route_var;
    protected $title;
    ...
    {
        $this->route = 'styles';
        $this->route_var = 'style';
        $this->title = 'STYLES';
        $this->view_path = 'tb_sys_mf_style';
    }

    public function index(Request $r)
    {
        store_audit(Auth::user()->id, 'STYLES');
        $name = $r['name'];
        $data = tb_sys_mf_style::when(isset($name), function ($q) use ($name) {
            return $q->where('tb_sys_mf_style.name', 'like', '%' . $name . '%');
        })
            ->sortable()
    ...
    public function store(tb_sys_mf_style_request $r)
    {
        $validatedData = $r->validated();
        $datum = new tb_sys_mf_style();
    ...
    {
        $datum = tb_sys_mf_style::findOrFail($id);
    ...
    public function update(tb_sys_mf_style_request $r, $id)
    {
        $datum = tb_sys_mf_style::findOrFail($id);
        $validatedData = $r->validated();
        $datum->fill($validatedData);
